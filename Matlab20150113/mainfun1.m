function []=mainfun1(obj,event,parameter)
%for kk =1:10000
tic;
...
%         roi = inimg;
        ...
        [angle_list,otherpie_centroid,mypie]=findmypie(spiral_obj,Idx,N,angle_list);
        
%         otherpie_list=[otherpie_list;otherpie];
        if ~isempty(mypie)
            mypie_centroid = mypie;
        end       
        
        if ~isempty(otherpie_centroid)
            if stp==1
...
%      save format.mat Angle mypie_centroid  closest_pie_centriod  time_img
    ...
    localdata.opl = otherpie_centroid_list;
    localdata.angle = Angle;
    localdata.mpc = mypie_centroid; 
    localdata.time = time_img ;
    obj.UserData = localdata; 
%     localdata=[];
    toc;
end
end