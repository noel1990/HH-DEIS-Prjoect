...
%      empty=imread('http://ideax3.hh.se/axis-cgi/jpg/image.cgi?resolution=800x600');
%     save matlab.mat empty
    ...
    mrect =     [1    1   size(imrgb,2) size(imrgb,1)]; thresh=0.4;%entire image as rectangle...
    ...
    %use kmeans find sutiable cluter
    
    
    if Number_of_spiral_obj<=3;   %spiral less than 4, again
        continue;
    elseif Number_of_spiral_obj>=50;
       
        disp(sprintf('no pie in the area'));
        continue;
    else                          %spiral more than 4,seperate
        ...
        if restart    %seperate fail, again
            restart=0;
            continue;
        end
        ...
        %find my pie and update the datatable
        %if no my pie in the area,restart
        %if no other pie in the area ,continue
